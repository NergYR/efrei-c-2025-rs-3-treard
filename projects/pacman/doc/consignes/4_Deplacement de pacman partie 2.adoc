...
Par contre, si vous appuyez sur la flèche du bas immédiatement, Pacman sortira par le bas.
On ne veut pas avoir à appuyer à nouveau sur la touche au moment exacte ou Pacman passe au-dessus du chemin.

image::resources/4.1.png[]

=== Continuer d'avancer

Pour maintenir un cap, nous avons une notion de direction, représenté sur les deux axes par `+dirX+` et `+dirY+`. +
...
Si l'utilisateur entre une commande, vous la sauvegardez puis vous la traitez.
Si aucune commande n'est entrée par l'utilisateur, faites comme s'il avait entré la commande que vous avez sauvegardée.