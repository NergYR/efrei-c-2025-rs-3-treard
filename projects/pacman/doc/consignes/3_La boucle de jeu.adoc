...
À chaque fois que vous faites une update, cela met fin à un cycle de la boucle, update est donc la dernière fonction à appeler dans votre boucle principale.

À chaque cycle, chaque entité du jeu (Pacman et les fantômes) auront l'occasion d'effectuer un déplacement.
...
La partie est gagné si Pacman a mangé toutes les Pacgums, créez une fonction `+bool win(const char ** level)+` qui vérifie cela.
...
Cependant, la fonction `+win+` doit seulement lire le contenu du niveau sans le modifier.
En le déclarant constant, on ajoute une sécurité qui nous empêchera de modifier par erreur.
En plus, cela rajoute de la clarté en indiquant explicitement à un autre développeur que `+win+` ne modifiera pas `+level+`.
On essaiera donc au maximum de rendre les paramètres constants.

=== Condition de défaite

La partie est perdu si Pacman et un fantôme se retrouvent sur la même case.
Vous vous inquiéterez de cela une fois que nous aurons au moins un fantôme qui se déplace.